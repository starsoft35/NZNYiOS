...
@interface CYMyUserInfoGenderVC ()

@end

@implementation CYMyUserInfoGenderVC
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    // 选中状态
    if ([self.dataArray[indexPath.row][@"title"] isEqualToString:self.onlyUser.Gender]) {
...
- (void)requestChangeGenderWithGender:(NSString *)gender{
    ...
    // 请求数据：修改性别
    NSString *newUrl = [NSString stringWithFormat:@"%@?userId=%@&gender=%@",cModifyGenderUrl,self.onlyUser.userID,gender];
    ...
    [CYNetWorkManager postRequestWithUrl:tempUrl params:nil progress:^(NSProgress *uploadProgress) {
        ...
        if ([code isEqualToString:@"0"]) {
            ...
            NSLog(@"修改性别：获取失败:responseObject:%@",responseObject);
            NSLog(@"修改性别：获取失败:responseObject:res:msg:%@",responseObject[@"res"][@"msg"]);
