//
//  CYUserRegisterProtocolVC.m
//  nzny
//
//  Created by 男左女右 on 2017/3/9.
//  Copyright © 2017年 nznychina. All rights reserved.
//

#import "CYUserRegisterProtocolVC.h"

@interface CYUserRegisterProtocolVC ()

@end

@implementation CYUserRegisterProtocolVC
...
- (void)loadData{
    ...
    // 网络请求：用户注册协议
    [CYNetWorkManager getRequestWithUrl:cRegistrationUrl params:nil progress:^(NSProgress *uploadProgress) {
        NSLog(@"获取用户注册协议进度：%@",uploadProgress);
        ...
        if ([code isEqualToString:@"0"]) {
            NSLog(@"用户注册协议：获取成功！：%@",responseObject);
            ...
            NSLog(@"用户注册协议：获取失败:responseObject:%@",responseObject);
            NSLog(@"用户注册协议：获取失败:responseObject:res:msg:%@",responseObject[@"res"][@"msg"]);
        ...
        NSLog(@"用户注册协议：请求失败！失败原因：error：%@",error);
