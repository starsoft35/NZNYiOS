...
@interface CYOfflineActivityVC ()

@end

@implementation CYOfflineActivityVC
...
- (void)loadData{
    ...
    // 网络请求：线下活动列表
    [CYNetWorkManager getRequestWithUrl:cOfflineActivityListUrl params:params progress:^(NSProgress *uploadProgress) {
        NSLog(@"获取线下活动列表进度：%@",uploadProgress);
        ...
        if ([code isEqualToString:@"0"]) {
            ...
            NSLog(@"线下活动列表：获取失败:responseObject:%@",responseObject);
            NSLog(@"线下活动列表：获取失败:responseObject:res:msg:%@",responseObject[@"res"][@"msg"]);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return (670.0 / 1334.0) * cScreen_Height;
}
